/**
 * 推广金「转余额后结算中」兜底（今华有肽 C 端）
 *
 * 背景（2026-09-16 用户反馈）：后端 `POST /api/wallet/convert`（type=PROMOTION）会把钱包里的
 * `pendingPromotion` 直接清零，而「尚未转出的推广金」要等后端定时任务（约 1 小时）重新累计回
 * `pendingPromotion`。这段窗口期内，前端按「pendingPromotion + 冻结中」算出的推广金合计会变成 0：
 * 用户明明还有一笔在冻结/待到账，个人页与推广页却显示 0，只能等后端轮询才恢复 → 极易起纠纷。
 *
 ...
 *   3. 兜底值有效期 3 小时（覆盖后端约 1 小时的入账任务），过期自动失效，避免长期显示偏大。
 */
