<template>
  <view class="lucky-wheel-box" :style="{ width: size + 'px', height: size + 'px' }">
    <!-- 转盘画布：切图打底 + 奖品叠加，整体 rotate 转动 -->
    <canvas
      type="2d"
      id="lucky-wheel-canvas"
      canvas-id="lucky-wheel-canvas"
      class="lucky-wheel-canvas"
      :style="{ width: size + 'px', height: size + 'px' }"
    />
    <!-- 顶部指针（独立切图，默认代码绘制红色三角；提供 pointerImage 时用图覆盖） -->
    <view class="lucky-pointer" :style="{ width: size + 'px', height: size + 'px' }">
      <image v-if="pointerImage" :src="pointerImage" class="lucky-pointer-img" mode="aspectFit" />
      <view v-else class="lucky-pointer-tri" />
    </view>
    <!-- 中心抽奖按钮（独立切图或代码绘制） -->
    <view class="lucky-wheel-btn" :style="{ width: btnSize + 'px', height: btnSize + 'px' }">
      <image v-if="btnImage" :src="btnImage" class="lucky-btn-img" mode="aspectFit" @click="handleBtnClick" />
      <view v-else class="lucky-btn-default" @click="handleBtnClick"><text class="lucky-btn-text">抽奖</text></view>
    </view>
  </view>
</template>

<script setup lang="ts">
import { getCurrentInstance, onMounted, onUnmounted, ref, watch } from 'vue'
import type { LuckyPrizeVO } from '@/types/lucky'

const props = withDefaults(defineProps<{
  /** 奖品格列表（驱动叠加奖品文字）。 */
  prizes?: LuckyPrizeVO[]
  /** 圆盘直径（px），默认 300。 */
  size?: number
  /** 中心按钮半径（px），默认 34。 */
  btnRadius?: number
  /** 转盘切图 URL（圆盘打底；为空时代码绘制纯色盘兜底）。 */
  wheelImage?: string
  /** 指针切图 URL（空则代码绘制）。 */
  pointerImage?: string
  /** 中心按钮切图 URL（空则代码绘制）。 */
  btnImage?: string
}>(), {
  prizes: () => [],
  size: 300,
  btnRadius: 34,
  wheelImage: '',
  pointerImage: '',
  btnImage: '',
})

const emit = defineEmits<{ (e: 'start'): void; (e: 'end', index: number): void }>()

const instance = getCurrentInstance()
const size = ref(props.size)
const btnSize = ref((props.btnRadius || 34) * 2)

let canvasNode: any = null
let ctx: any = null
let dpr = 1
let timer: ReturnType<typeof setTimeout> | null = null
let wheelImg: any = null // 已加载的切图

/** 当前转动角度（度）。 */
let angle = 0
let animating = false
/** 格子背景色盘（切图为空时的兜底盘）。 */
const PALETTE = ['#ffd7d7', '#fff0c9', '#d7f5e0', '#dbe7ff', '#f7e0ff', '#ffe6c9']

function initCanvas(): void {
  uni.createSelectorQuery().in(instance?.proxy).select('#lucky-wheel-canvas')
    .fields({ node: true, size: true })
    .exec((res) => {
      const info = res?.[0]
      if (!info || !info.node) return
      canvasNode = info.node
      const w = info.width || props.size
      const h = info.height || props.size
      canvasNode.width = w * dpr
      canvasNode.height = h * dpr
      ctx = canvasNode.getContext('2d')
      ctx.scale(dpr, dpr)
      size.value = w
      btnSize.value = (props.btnRadius || 34) * 2
      // 先加载切图（若有），加载完成后再首绘。
      if (props.wheelImage) {
        loadImage(props.wheelImage).then((img) => {
          wheelImg = img
          draw()
        }).catch(() => { wheelImg = null; draw() })
      } else {
        draw()
      }
    })
}

/** 加载一张图片（canvas.createImage）。 */
function loadImage(src: string): Promise<any> {
  return new Promise((resolve, reject) => {
    if (!canvasNode) { reject(new Error('canvas 未就绪')); return }
    const img = canvasNode.createImage()
    img.onload = () => resolve(img)
    img.onerror = () => reject(new Error('图片加载失败'))
    img.src = src
  })
}

/** 绘制圆盘：切图打底 + 按 prizes 叠加奖品文字，整体随 angle 旋转。 */
function draw(): void {
  if (!ctx || !canvasNode) return
  const center = size.value / 2
  const radius = center
  ctx.clearRect(0, 0, size.value, size.value)
  ctx.save()
  ctx.translate(center, center)
  ctx.rotate((angle * Math.PI) / 180)

  const prizes = props.prizes
  const count = prizes.length || 1
  const sector = (Math.PI * 2) / count

  if (wheelImg) {
    // 有切图：绘制圆盘底图
    ctx.drawImage(wheelImg, -radius, -radius, size.value, size.value)
  } else {
    // 无切图兜底：绘制纯色圆盘分扇区
    prizes.forEach((prize, i) => {
      const startAngle = i * sector
      const endAngle = startAngle + sector
      ctx.beginPath()
      ctx.moveTo(0, 0)
      ctx.arc(0, 0, radius, startAngle, endAngle)
      ctx.closePath()
      ctx.fillStyle = PALETTE[i % PALETTE.length]
      ctx.fill()
      ctx.strokeStyle = '#ffffff'
      ctx.lineWidth = 2
      ctx.stroke()
    })
  }

  // 叠加奖品文字（沿扇区外侧径向布置）
  prizes.forEach((prize, i) => {
    ctx.save()
    ctx.rotate(i * sector + sector / 2)
    ctx.textAlign = 'right'
    ctx.textBaseline = 'middle'
    ctx.fillStyle = '#5a4a42'
    ctx.font = 'bold 14px sans-serif'
    ctx.fillText(prize.name || '', radius - 16, 0)
    ctx.restore()
  })

  ctx.restore()
}

/** easeOutQuad 缓动。 */
function easeOutQuad(t: number): number {
  return 1 - (1 - t) * (1 - t)
}

/** 转到目标角度：让 index 格中心对齐顶部指针。 */
function rotateTo(index: number, duration: number): void {
  const count = props.prizes.length || 1
  const sector = 360 / count
  // 4 圈以上余量，目标格中心对准顶部（0 位）。
  const base = 360 * 4 + (360 - index * sector - sector / 2)
  const start = angle
  const delta = ((base - (start % 360) + 360) % 360) + 360
  const end = start + delta
  const startTime = Date.now()

  const step = (): void => {
    const t = Math.min(1, (Date.now() - startTime) / duration)
    angle = start + delta * easeOutQuad(t)
    draw()
    if (t < 1) {
      timer = setTimeout(step, 16)
    } else {
      angle = end
      draw()
      animating = false
      emit('end', index)
    }
  }
  timer = setTimeout(step, 16)
}

/** 开始转动（先随机转一格）。 */
function play(): void {
  if (animating) return
  animating = true
  const idx = Math.floor(Math.random() * (props.prizes.length || 1))
  rotateTo(idx, 2500)
}

/** 停止到指定格（重定向）。 */
function stop(index: number): void {
  if (timer) { clearTimeout(timer); timer = null }
  animating = true
  rotateTo(index, 1200)
}

function handleBtnClick(): void {
  emit('start')
}

watch(() => props.prizes, () => draw(), { deep: true })

onMounted(() => {
  dpr = uni.getSystemInfoSync().pixelRatio || 2
  setTimeout(() => initCanvas(), 60)
})

onUnmounted(() => {
  if (timer) { clearTimeout(timer); timer = null }
})

defineExpose({ play, stop })
</script>

<style scoped>
.lucky-wheel-box { position: relative; margin: 0 auto; }
.lucky-wheel-canvas { position: absolute; left: 0; top: 0; }
.lucky-pointer { position: absolute; left: 0; top: 0; pointer-events: none; z-index: 2; }
.lucky-pointer-img { width: 100%; height: 100%; }
.lucky-pointer-tri {
  position: absolute;
  left: 50%;
  top: 0;
  width: 0;
  height: 0;
  border-left: 10px solid transparent;
  border-right: 10px solid transparent;
  border-top: 22px solid #e5322d;
  transform: translateX(-50%);
}
.lucky-wheel-btn { position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); z-index: 3; }
.lucky-btn-img { width: 100%; height: 100%; border-radius: 50%; }
.lucky-btn-default {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 100%;
  height: 100%;
  border-radius: 50%;
  background: #e5322d;
  box-shadow: 0 4rpx 12rpx rgba(229, 50, 45, .4);
}
.lucky-btn-text { color: #fff; font-size: 26rpx; font-weight: 600; }
</style>
