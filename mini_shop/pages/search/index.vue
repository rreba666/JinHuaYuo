...
/** 打开商品详情页。 */
function goDetail(id: string | number): void { uni.navigateTo({ url: `/pages/product/detail?id=${id}` }) }
        ...
        <view v-for="item in products" :key="String(item.id)" class="card" @click="goDetail(item.id)">
          ...
          <view class="card-body">
            ...
            <view class="card-bottom">
              <view class="price"><text class="currency">¥</text><text class="amount">{{ Number(item.price || 0).toFixed(2) }}</text></view>
...
.nav { display: flex; align-items: center; height: 104rpx; padding: 0 24rpx; box-sizing: border-box; border-bottom: 1px solid #f0f0f0; }
