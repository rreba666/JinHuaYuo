...
const bodyTop = computed(() => menuTop.value + menuHeight.value + 8)
...
async function loadList(): Promise<void> {
  try { items.value = await getCartList() } catch (e) { console.error('购物车列表加载失败:', e) } finally { loading.value = false }
}
...
async function onChangeQty(item: CartItem, delta: number): Promise<void> {
  ...
  if (n < 1 || n > (item.stock || 9999)) { uni.showToast({ title: n < 1 ? '最少1件' : '库存不足', icon: 'none' }); return }
  if (busy.value) return; busy.value = true
  try { await updateQuantity(item.cartId, n); item.quantity = n } catch (e) { /* */ } finally { busy.value = false }
}
...
function toggleEditMode(): void { editMode.value = !editMode.value }
...
onShow(() => { loading.value = true; loadList() })
</script>
...
<template>
  <view class="pg">
    ...
    <view class="nav" :style="navStyle">
      ...
      <text class="nav-act" @click="toggleEditMode">{{ editMode ? '完成' : '管理' }}</text>
    </view>
    ...
    <view class="bd" :style="{ paddingTop: bodyTop + 'px' }">
      ...
      <scroll-view v-else class="lst" scroll-y>
          ...
          <view class="info">
            ...
            <view class="ifoot">
              <view v-if="!editMode" class="stp">
                ...
                <view class="stp-b" @click="onChangeQty(it, -1)"><image class="stp-i" :src="it.quantity <= 1 ? '/static/cart/del_no.png' : '/static/cart/del.png'" mode="aspectFit" /></view>
...
.nav-act { position: absolute; right: 26rpx; color: #666; font-size: 28rpx; }
