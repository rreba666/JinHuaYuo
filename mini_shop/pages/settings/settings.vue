...
<template>
  <view class="page">
    ...
    <scroll-view class="scroll" scroll-y :style="{ paddingTop: (menuTop + menuHeight + 12) + 'px' }">
      ...
      <!-- 收货地址（待后端） -->
      <view class="card">
        <view class="row" @click="uni.navigateTo({ url: '/pages/settings/address/address' })">
          <text class="row-label">收货地址</text>
          <text class="row-value muted">地址簿管理</text>
          <text class="row-arrow">›</text>
        </view>
      </view>
      ...
      <!-- 提现银行卡（待后端） -->
      <view class="card">
        <view class="row" @click="uni.navigateTo({ url: '/pages/settings/bank-card/bank-card' })">
          <text class="row-label">提现银行卡</text>
          <text class="row-value muted">绑定银行卡管理</text>
          <text class="row-arrow">›</text>
        </view>
      </view>
      <PageWatermark />
