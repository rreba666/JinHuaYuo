...
function openDetail(item: ProductFavorite): void {
  uni.navigateTo({ url: `/pages/product/detail?id=${encodeURIComponent(String(item.id))}` })
}
      ...
      <view v-for="item in list" :key="item.id" class="fav-card" @click="openDetail(item)">
        ...
        <view class="fav-info">
          ...
          <view class="fav-bottom">
            <view class="fav-price"><text class="currency">¥</text><text class="amount">{{ formatAmount(item.price) }}</text></view>
