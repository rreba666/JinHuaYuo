...
import { getUserProfile, getWalletInfo, searchUser, transferWallet, withdrawWallet, type UserProfile, type UserSearchVO, type WalletInfo } from '@/api/user'
...
async function loadPage(): Promise<void> {
  ...
  await Promise.all([loadWallet(), loadTransferAuthStatus()])
  openWalletNotice()
}
...
async function executeWithdraw(amount: number): Promise<void> {
  ...
  try {
    ...
    await loadWallet()
    uni.showToast({ title: '零钱提现已提交', icon: 'success' })
  } catch (error) {
      ...
      <view class="page-content">
        ...
        <view v-show="currentTab === 'withdraw'" class="panel-card">
          ...
          <view v-else-if="needAuth" class="auth-banner">
            <text class="auth-text">{{ transferAuthState === 'WAIT_USER_CONFIRM' ? '授权未完成，请再次确认授权' : '首次提现前需完成免确认收款授权，授权后提现直通到账' }}</text>
...
.nav { position: fixed; right: 0; left: 0; z-index: 20; display: flex; align-items: center; padding: 0 32rpx; box-sizing: border-box; background: #fff; box-shadow: 0 1rpx 0 rgba(16, 24, 40, .04); }
.back-button { width: 34rpx; height: 34rpx; border-top: 4rpx solid #222; border-left: 4rpx solid #222; transform: rotate(-45deg); }
...
.hero-card { position: relative; overflow: hidden; margin-top: 20rpx; padding: 36rpx 30rpx 32rpx; border-radius: 28rpx; background: linear-gradient(135deg, #ff6a2b 0%, #ff5a1f 100%); box-shadow: 0 14rpx 28rpx rgba(255, 90, 31, .22); color: #fff; }
.hero-card::after { position: absolute; right: -36rpx; bottom: -42rpx; width: 170rpx; height: 160rpx; border-radius: 50%; background: rgba(255, 255, 255, .12); content: ''; }
