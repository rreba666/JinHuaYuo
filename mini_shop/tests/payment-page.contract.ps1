...
$paymentPath = Join-Path $root "subpkg-order/payment/payment.vue"

if (-not (Test-Path -LiteralPath $paymentPath)) { throw "payment page file missing" }
if (-not (Select-String -LiteralPath $cartPath -Pattern "subpkg-order/payment/payment" -SimpleMatch)) { throw "cart entry missing" }
if (-not (Select-String -LiteralPath $cartPath -Pattern "cartIds" -SimpleMatch)) { throw "cartIds missing" }
if (-not (Select-String -LiteralPath $pagesPath -Pattern "subpkg-order/payment/payment" -SimpleMatch)) { throw "route missing" }
