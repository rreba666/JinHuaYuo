...
import { Box, Delete, Edit, View } from '@element-plus/icons-vue'
 ...
 * 商品详情里「所有 SKU 库存之和」。
 *
 * ⚠️ 商品详情接口返回的 `ProductDetailVO` **不包含** `totalStock`，所以详情里的「库存」只能按 SKU 求和；
 * 商品列表的「库存」= `ProductListVO.totalStock`，后端自 2026-09-16 起已改为**查询时实时聚合**
 * （`SUM(enabled=1 且未软删 SKU 的 stock)`，**不含** locked_stock），与本合计同口径 —— 两处数值应当一致。
 */
const detailSkuStockSum = computed(() => (store.detail?.skuList || []).reduce((sum, sku) => sum + Number(sku.stock || 0), 0))
...
function createEmptyForm(): AdminProductSaveDTO {
  return { id: undefined, name: '', categoryId: '', mainImage: '', images: [], videoUrl: '', description: '', descriptionTitle: '', originPlace: '', detailImages: [], detailPosterUrl: '', promotionFund: 0, promotionEnabled: 1, dividendFund: 0, dividendEnabled: 1, peptideEnabled: 0, emergencyPoolEnabled: 0, emergencyPoolAmount: 0, status: 1, isRecommended: 0, recommendTextEnabled: 0, sortOrder: 0, skuList: [] }
}
...
/**
 * 查某个分类的「默认支持肽金券抵扣」（0/1，分类不存在返回 null）。
 * 商品保存时后端也会做同样的兜底（未显式传 `peptideEnabled` → 取分类默认并**落库到商品**），
 * 这里同步一次只是让运营在提交前就看到实际会落库的值。
 */
function findCategoryPeptideDefault(categoryId: string): ProductStatus | null {
  const target = String(categoryId || '')
  if (!target) return null
  const walk = (nodes: CategoryNode[]): ProductStatus | null => {
    for (const node of nodes) {
      if (String(node.id) === target) return normalizeBinary(node.peptideEnabled ?? 0)
...
function handleCategoryChange(categoryId: string): void {
  ...
  form.peptideEnabled = fallback
}

/** 复制详情数据到编辑表单，避免弹窗修改列表原数据。 */
function fillForm(detail?: ProductDetail): void {
  const status = normalizeBinary(detail?.status ?? 1)
  Object.assign(form, detail ? { id: detail.id, name: detail.name, categoryId: detail.categoryId, mainImage: detail.mainImage, images: [...(detail.images || [])], videoUrl: detail.videoUrl || '', description: detail.description || '', descriptionTitle: detail.descriptionTitle || '', originPlace: detail.originPlace || '', detailImages: [...(detail.detailImages || [])], detailPosterUrl: detail.detailPosterUrl || '', promotionFund: detail.promotionFund ?? 0, promotionEnabled: normalizeBinary(detail.promotionEnabled), dividendFund: detail.dividendFund ?? 0, dividendEnabled: normalizeBinary(detail.dividendEnabled), peptideEnabled: normalizeBinary(detail.peptideEnabled ?? 0), emergencyPoolEnabled: normalizeBinary(detail.emergencyPoolEnabled ?? 0), emergencyPoolAmount: detail.emergencyPoolAmount ?? 0, status, isRecommended: status === 1 ? normalizeBinary(detail.isRecommended) : 0, recommendTextEnabled: status === 1 && normalizeBinary(detail.isRecommended) === 1 ? normalizeBinary(detail.recommendTextEnabled) : 0, sortOrder: detail.sortOrder || 0, skuList: (detail.skuList || []).map((sku) => ({ ...sku, id: sku.id == null ? undefined : String(sku.id), enabled: normalizeBinary(sku.enabled) })) } : createEmptyForm())
...
/** 跳转「库存对账 → 库存台账」，按 SKU 查看库存变动流水（只读，来自商品详情 SKU 行入口）。 */
function openStockLedger(skuId: string | number | undefined): void {
  const id = String(skuId ?? '').trim()
  if (!id) {
    ElMessage.warning('该 SKU 缺少 ID，无法查看库存台账')
    return
  }
  void router.push({ path: '/stock', query: { skuId: id } })
}

/**
 * 按**商品 ID** 跳转「库存对账 → 库存台账」。
 * 商品列表只有 `ProductListVO.id`（没有 skuId），后端支持只传 productId：
 * 该商品恰好 1 个启用 SKU 时直接出账并返回 `resolvedFromProduct=true`；
 * 多个启用 SKU 会返回 `code=1000` 并在 message 里列出可选 skuId（台账页会原样提示）。
 */
function openStockLedgerByProduct(productId: string | number | undefined): void {
  const id = String(productId ?? '').trim()
  if (!id) {
    ElMessage.warning('该商品缺少 ID，无法查看库存台账')
    return
  }
  void router.push({ path: '/stock', query: { productId: id } })
}
  ...
  <section class="page-container">
    ...
    <el-dialog v-model="formVisible" class="product-form-dialog" :title="editingId ? '编辑商品' : '新增商品'" width="min(1100px, calc(100vw - 32px))" top="2vh" append-to-body>
      <el-form ref="formRef" class="product-form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="商品分类" prop="categoryId"><el-select v-model="form.categoryId" placeholder="请选择分类" :disabled="isSpecialCategory" @change="handleCategoryChange"><el-option v-for="option in categoryOptions" :key="option.id" :label="option.label" :value="option.id" /></el-select><p v-if="isSpecialCategory" class="special-category-tip">复购专区商品不能改分类；如需退出专区，请删除该商品后重新创建。</p></el-form-item>
...
.special-category-tip { width: 100%; margin: 4px 0 0; color: #b88230; font-size: 13px; line-height: 20px; }
...
.special-fund-tip strong { display: block; margin-bottom: 4px; color: #96651f; }
.special-fund-tip span { display: block; }
...
/* 商品详情「可售库存」右侧的 SKU 合计对照（排查 totalStock 与 SKU 实时库存不一致用） */
.stock-hint { margin-left: 8px; color: var(--el-text-color-secondary); font-size: 12px; }
.stock-cell { border-bottom: 1px dashed var(--el-border-color); cursor: help; }
/* 锁定库存 > 0 时高亮（对账关注点） */
.stock-locked { color: var(--el-color-warning); font-weight: 600; }
