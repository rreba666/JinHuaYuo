 ...
 * 商品详情里「所有 SKU 库存之和」。
 *
 * 用途：后端 `totalStock` 是**保存商品时汇总**的冗余字段（api-docs：`totalStock = 所有启用 SKU 的库存之和`），
 * 而「库存台账」读的是 `product_sku.stock` 的**实时值** —— 两者不一致就说明 `product.total_stock`
 * 没有随库存变动（下单/取消/退款回补等）同步。把两个数摆在一起，可一眼判断是后端冗余字段没更新，
 * 而不是"台账查错了 SKU"。
 */
