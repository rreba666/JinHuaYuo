...
function normalizeProductDetail(detail: ProductDetail): ProductDetail {
  return {
    ...
    detailPosterUrl: resolveMediaUrl(detail.detailPosterUrl),
...
export async function getAdminProductDetail(productId: string): Promise<ProductDetail> {
  const response = await request.get<ProductResponse<ProductDetail>>(`/api/admin/product/detail/${productId}`, {
    transformResponse: [(data) => parseProductJson(data)],
  })
...
export async function saveAdminProduct(payload: AdminProductSaveDTO): Promise<void> {
  const response = await request.post<ProductResponse<null>>('/api/admin/product/save', payload, {
    transformResponse: [(data) => parseProductJson(data)],
  })
