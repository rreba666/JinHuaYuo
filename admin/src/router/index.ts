...
const SUPER_AND_SERVICE: AdminRole[] = ['SUPER_ADMIN', 'CUSTOMER_SERVICE']
const SUPER_AND_FINANCE: AdminRole[] = ['SUPER_ADMIN', 'FINANCE']
const ALL: AdminRole[] = ['SUPER_ADMIN', 'CUSTOMER_SERVICE', 'FINANCE']
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          meta: { title: '首屏与品牌', permission: ['homepage:read'], roles: SUPER_ADMIN, requiresAuth: true },
        },
        {
          ...
          meta: { title: '底部推荐', permission: ['homepage:read'], roles: SUPER_ADMIN, requiresAuth: true },
        },
        ...
        { path: 'homepage/preview', redirect: '/homepage' },
        {
          ...
          meta: { title: '用户管理', permission: ['user:read'], roles: SUPER_AND_SERVICE, requiresAuth: true },
        },
        {
          path: 'products',
          name: 'Products',
          component: () => import('@/views/products/index.vue'),
          meta: { title: '商品管理', permission: ['product:read'], roles: SUPER_AND_SERVICE, requiresAuth: true },
        },
        {
          path: 'categories',
          name: 'Categories',
          component: () => import('@/views/categories/index.vue'),
          meta: { title: '分类管理', permission: ['category:read'], roles: SUPER_ADMIN, requiresAuth: true },
        },
        {
          path: 'shops',
          name: 'Shops',
          component: () => import('@/views/shops/index.vue'),
          meta: { title: '门店管理', permission: ['shop:read'], roles: SUPER_AND_SERVICE, requiresAuth: true },
        },
        {
          path: 'staff',
          name: 'Staff',
          component: () => import('@/views/staff/index.vue'),
          meta: { title: '店员管理', permission: ['staff:read'], roles: SUPER_ADMIN, requiresAuth: true },
        },
        {
          path: 'admins',
          name: 'Admins',
        ...
        },
        {
          ...
          meta: { title: '普通订单', permission: ['order:read'], roles: SUPER_AND_SERVICE, requiresAuth: true },
        },
        {
          path: 'orders/pickup',
          name: 'PickupOrders',
          component: () => import('@/views/orders/index.vue'),
          meta: { title: '自提订单', permission: ['order:read'], roles: SUPER_AND_SERVICE, requiresAuth: true },
        },
        {
        ...
        },
        {
          ...
          meta: { title: '钱包管理', permission: ['wallet:read'], roles: SUPER_AND_FINANCE, requiresAuth: true },
        },
        {
          path: 'transfers',
          name: 'Transfers',
          component: () => import('@/views/transfers/index.vue'),
          meta: { title: '余额转账记录', permission: ['transfer:read'], roles: SUPER_AND_FINANCE, requiresAuth: true },
        },
        {
          path: 'withdraw',
          name: 'Withdraw',
          component: () => import('@/views/withdraw/index.vue'),
          meta: { title: '提现审核', permission: ['withdraw:read'], roles: SUPER_AND_FINANCE, requiresAuth: true },
        },
        {
          path: 'logs/verify',
          name: 'VerifyLogs',
...
function getSafeRedirect(path: unknown): string {
  return typeof path === 'string' && path.startsWith('/') && !path.startsWith('//') ? path : '/merchant'
}

/** 商户系统默认落地页：商户业务台。 */
function homeForScope(): string {
  return '/merchant'
}
...
router.beforeEach((to) => {
  ...
  if (to.name === 'Login' && authStore.isAuthenticated) {
    return getSafeRedirect(to.query.redirect || homeForScope())
  }
  if (requiresAuth && !authStore.isAuthenticated) {
    return { name: 'Login', query: { redirect: to.fullPath } }
  }
  // 角色守卫：已登录但角色不在路由允许列表中时，跳回商户业务台。
  if (requiresAuth && authStore.isAuthenticated) {
