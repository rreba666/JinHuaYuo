    ...
    <div class="image-grid">
      <div v-for="(url, index) in modelValue" :key="`${url}-${index}`" class="image-tile">
        <el-image
          ...
          class="image-preview"
          :src="url"
          :preview-src-list="modelValue.filter(Boolean)"
          :initial-index="modelValue.filter(Boolean).indexOf(url)"
          fit="cover"
          preview-teleported
        >
          <template #error><div class="image-preview--error">图片加载失败</div></template>
        </el-image>
        <div v-else class="image-preview image-preview--empty"><span class="image-plus">+</span></div>
      ...
      <el-upload
        ...
        :accept="accept"
      >
        <div class="image-preview image-preview--empty"><span class="image-plus">+</span></div>
      </el-upload>
    </div>
  </div>
</template>
